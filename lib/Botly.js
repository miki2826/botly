...
Botly.prototype.getUserProfile = function (senderId, cb) {
    const USER_URL = `${FB_URL}${senderId}`;
...
function _handleOptIn(message) {
    if (message.message.optin) {
        let optin = message.message.optin.ref;
...
function _handleDelivery(message) {
    if (message.message.delivery) {
        let mids = message.message.delivery.mids;
...
function _handlePostback(message) {
    if (message.message.postback) {
        let postback = message.message.postback.payload;
